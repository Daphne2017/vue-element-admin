...
<template>
  <div class="el-wrapper-box">
    ...
    >
      <template #tableBtn>
        <el-button type="success" icon="el-icon-plus" @click="addNewTag('new')">新增标签</el-button>
      </template>
      ...
      <template #operate="{ scope: { row} }">
        <div class="operate-btn-wrap">
          <el-button size="small" type="info" @click="addNewTag('edit',row)">编辑</el-button>
        </div>
...
import { tagLibraryListApi, sumbitTagApi, getRelatedGameByTagIdApi } from '@/api/tagManage.js'
import {
  formFields,
  formAttr,
  column,
...
export default {
  ...
  methods: {
    ...
    async addNewTag(mode, row) {
      ...
      const res = await getRelatedGameByTagIdApi(row.id)
      this.formData.games = res.data.map(item => item.gameInfo && item.gameInfo.gameName).join(',')
    ...
    async confirm() {
      try {
        await sumbitTagApi({ ...this.formData }) // 新增/编辑
        this.formLoading = false
